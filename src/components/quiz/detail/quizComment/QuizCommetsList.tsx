...
import QuizCommentsCancelBtn from './QuizCommentsCancelBtn';
          ...
          <div className="w-full flex flex-col gap-2">
            ...
            {getformattedDate(new Date(new Date(comment.created_at).getTime() + 9 * 60 * 60 * 1000).toString())}
                ...
                <p className="text-xs text-gray-500">
                  {Math.floor(
                    getHoursDifference(new Date(new Date(comment.created_at).getTime() + 9 * 60 * 60 * 1000).toString())
                  )}
                  시간 전
                </p>
              </div>
              <QuizCommentsCancelBtn />
            </div>
