...
export const algorithmSchema = z.object({
  ...
  explanation: z.string().refine((val) => val.replace(htmlTagRegex, '').trim().length > 0, {
    message: '본인이 풀었던 코드를 입력해주세요.'
  }),
  newLearn: z.string().refine((val) => val.replace(htmlTagRegex, '').trim().length > 0, {
    message: '배운 점을 입력해주세요.'
  })
});
